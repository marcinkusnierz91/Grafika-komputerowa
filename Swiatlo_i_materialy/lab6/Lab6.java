...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	private void pyramid(GL2 gl2) {
		...
		for (int i = 0; i < 5; i++) {
			float j = i - 1;
			float k = i;
			float angleJ = j * 2 * (float) Math.PI / 5;
			float angleK = k * 2 * (float) Math.PI / 5;
	...
	public void init(GLAutoDrawable drawable) {
		...
		float[] diffuseColor = {1.0f, 0.6f, 0.8f, 1};   // Light pink diffuse light
		float[] specularColor = {1.0f, 0.5f, 0.0f, 1};  // Orange specular light
		float[] ambientColor = {0.0f, 0.0f, 1.0f, 1};   // Blue ambient light
